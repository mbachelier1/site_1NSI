...
!!! question "Comparer deux algorithmes de recherhce d'un élément dans un tableau"
...
## Exercices
### Exercice 1
Écrire le programme qui calcule le maximum et le minimum d'un tableau en utilisant l'algorithme proposé dans l'activité (recherche de maximum). Ajouter un compteur pour compter le nombre d'étapes réalisées par le programme et l'exécuter avec un tableau de 20 éléménts dans le pire des cas.  

!!! tip "Le pire des cas"
	Le pire des cas quand on recherche un maximum est un tableau trié dans l'ordre croissant, et pour la recherche du minimum, un tableau trié dans l'ordre décroissant.  
...
!!! tip "Le pire des cas"
	Le pire des cas quand on recherche un élémént est un tableau où le trouve à chaque étape, donc un tableau qui ne contient que les éléments que l'on cherche.




!!! note "Comparer les compléxités"
	Si le temps le permet, après avoir codé la recherche de valeurs avec les deux algorithmes de recherche de valeur (exercice 2), récupérer le nombre d'étapes en fonction de la taille de la liste et tracer les deux graphiques dans la même fenêtre.
...
### Exercice 4
Une liste contient des listes de coordonnées dans l'espace (x,y,z). On cherche le point le plus bas! Réalisez ce codage en recherchant la complexité maximum dans le pire des cas.

!!! example "Liste de coordonnées"
	Une telle liste pourrait être par exemple, `[(3,4,5),(0,0,0),(2,9,-3),(0,0,1),(0,1,1)]`. Le point le plus bas est celui dont la valeur de z est la plus faible. 
	Le pire des cas est celui ou on recontre le point recherché à la fin ou à chaque fois selon si on parcourt toute la liste ou non.  
